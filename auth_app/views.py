...
# -------------------------------------SUPER ADMIN-----------------------------------------
class SuperAdmin(APIView):
    def get(self, request, format=None):
        try:
            users = User.objects.filter(is_superuser=True)
            if not users:
                return Response({"message": "No superadmins found."}, status=status.HTTP_204_NO_CONTENT)
            serializer = AdminSerializer(users, many=True)
            return Response(serializer.data, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({"error": f"Failed to retrieve super admins: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class SuperAdminProfile(APIView):
    def get_object(self, superadmin_id):
        return get_object_or_404(User, id=superadmin_id)

    def get(self, request, superadmin_id, format=None):
        superadmin = User.objects.get(id = superadmin_id)
        serializer = AdminSerializer(superadmin)
        return Response(serializer.data, status = status.HTTP_200_OK)
    
    def patch(self, request, superadmin_id):
        user = self.get_object(superadmin_id)
...
class ProductAdminEdit(APIView):
    ...
    def get(self, request, productadmin_id, format=None):
        productadmin = self.get_object(productadmin_id)
    ...
    def delete(self, request, productadmin_id, format=None):
        product = self.get_object(productadmin_id)
        product.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
